...
- Fixed GitHub Copilot Business seats getting HTTP 403 on every model while the same token succeeds with a Chat client identity: default-identity chat requests that are denied are now retried once as `copilot-chat`, and `COPILOT_INTEGRATION_ID` pins the `Copilot-Integration-Id` header up front (default `copilot-developer-cli`) so the denied first attempt can be skipped; the 403 message names the identity and the remedies ([#11372](https://github.com/can1357/oh-my-pi/issues/11372)).
