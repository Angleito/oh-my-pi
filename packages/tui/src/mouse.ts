 ...
 * screen (see tui.ts MOUSE_TRACKING_ON), or — opt-in via `tui.mouse` — as
 * click-only capture on the normal buffer (MOUSE_TRACKING_INLINE_ON, no
 * motion reports). Consumers hit-test against their own rendered frame: the
 * frame paints from screen row 0, hence `row`/`col` are exposed 0-based for
 * direct indexing into rendered lines.
 */
